package com.itheima.spider.newsTencent;
...
public class TencentSpider {
    ...
    public static void addNews(List<News> newsList){
        for (News news : newsList) {
            ...
            //一旦数据保存到了MySQL中, 需要将新添加到数据库中url添加到redis的set集合中, 防止重复爬取
            Jedis jedis = JedisUtils.getJedis();
            jedis.sadd("bigData:spider:TencentSpider:itemUrl",news.getDocurl());
    ...
    private static List<News> parseNewsJson(String newsJson) {
        ...
        for (Map<String, Object> newsListDatum : newsListData) { //一个map就是一个新闻的对象
            ...
            //去重判断
            Jedis jedis = JedisUtils.getJedis();
            Boolean flag = jedis.sismember("bigData:spider:TencentSpider:itemUrl", itemUrl);
            jedis.close();
